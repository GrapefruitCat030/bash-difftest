...
- **3.4.2 Special Parameters**：Bash对某些特殊参数有扩展行为
...
- **3.5.3 Shell参数扩展**
  ...
  - `${parameter/pattern/replacement}` - 模式替换
  - `${parameter^}`, `${parameter^^}` - 大小写转换
  - `${parameter@operator}` - 参数变换操作符
...
- Bash支持更多的内置调试工具和选项# Bash Basic Shell Features
